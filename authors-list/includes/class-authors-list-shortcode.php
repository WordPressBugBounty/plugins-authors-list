...
class Authors_List_Shortcode {
	...
	public function __construct() {
		...
		add_shortcode( 'sk_authors_list', array( $this, 'shortcode' ) );
	...
	public function shortcode( $atts = false, $content = false ) {
		...
		$authors_ids = $authors_query->get_results();
		$authors_total = $authors_query->get_total();

		// Start output buffer.
		ob_start();

		$this->shortcode_content( $authors_ids, $settings, $atts, $authors_total );
	 ...
	 * @param array $settings    Shortcode attributes.
	 * @param array $atts        Shortcode attributes.
	 * @param array $authors_total Total authors.
	 */
	public function shortcode_content( $authors_ids, $settings, $atts, $authors_total ) {
		...
		<div class="authors-list-item authors-list-item-clearfix authors-list-col <?php echo esc_attr( $item_class ); ?>">
			...
			<div class="authors-list-item-main">
				...
				<?php if ( 'yes' === $settings['show_link'] ) : ?>
					<?php
						$link_text = esc_html__( 'View Posts &rarr;', 'authors-list' );
	...
	public function get_meta( $user_id = false, $name = false ) {
		...
		// No userdata, try get the author meta
		if ( ! $value ) {
			$value = get_the_author_meta( $name, $user_id );
		}
